...
def transactions():
    ...
    if request.method == "POST":
        ...
        # Coerce to int — sessionStorage and <select> values arrive as strings
        try:
            user_id  = int(user_id)
            item_id  = int(item_id)
...
def reports():
    ...
    cursor.execute(
        "SELECT COUNT(*) AS total_txn FROM transactions WHERE user_id=?", (user_id,)
    )
    ...
    cursor.execute(
        """
        SELECT category, SUM(quantity * unit_price) AS value, SUM(quantity) AS total_qty
        FROM items
        WHERE user_id=?
        GROUP BY category
        ORDER BY value DESC
